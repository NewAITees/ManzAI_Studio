...
def test_full_manzai_generation_flow(client):
    ...
    generate_response = client.post("/api/generate", json={"topic": "テスト", "model": "gemma3:4b"})
    ...
    script_for_synthesis = [
        {
            "speaker": "ツッコミ",
            "text": generate_data["script"][0]["text"],
            "speaker_id": 1,
        },
        {
            "speaker": "ボケ",
            "text": generate_data["script"][1]["text"],
            "speaker_id": 2,
        },
    ]

    synthesis_response = client.post("/api/synthesize", json={"script": script_for_synthesis})
...
def test_error_handling_and_recovery(client):
    ...
    with patch(
        "src.backend.app.routes.api.current_app.ollama_service.generate_manzai_script"
    ) as mock_generate:
        mock_generate.side_effect = Exception("Simulated error")

        # Call the generate endpoint
        error_response = client.post(
            "/api/generate", json={"topic": "エラーテスト", "model": "gemma3:4b"}
        )
    ...
    normal_response = client.post(
        "/api/generate", json={"topic": "回復テスト", "model": "gemma3:4b"}
    )
