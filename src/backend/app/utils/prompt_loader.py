...
class PromptLoader:
    """プロンプトローダークラス"""
    def __init__(self, prompts_dir: Optional[str] = None, templates_dir: Optional[str] = None):
        """
        Args:
    ...
    def get_prompt_by_id(self, prompt_id: str) -> Optional[Dict[str, Any]]:
        """IDでプロンプトを取得"""
        try:
            with open(os.path.join(self.prompts_dir, f"{prompt_id}.json"), 'r', encoding='utf-8') as f:
                return json.load(f)
