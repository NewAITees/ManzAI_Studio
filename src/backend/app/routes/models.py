...
def get_live2d_models() -> Dict[str, Any]:
    ...
    try:
        ...
        if not os.path.exists(LIVE2D_BASE_DIR):
            return jsonify({"models": [], "message": "モデルディレクトリが存在しません。"})
            ...
            if os.path.exists(model_json_path):
                try:
                    ...
                    model_info = {
                        ...
                        "thumbnail": f"/live2d/models/{model_name}/thumbnail.png"
                        if os.path.exists(os.path.join(model_dir, "thumbnail.png"))
                        else None,
                    }
        ...
        raise APIError(f"モデル一覧の取得中にエラーが発生しました: {e!s}", 500)
