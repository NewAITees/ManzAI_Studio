...
def get_prompts() -> Dict[str, Any]:
    ...
    try:
        ...
        if not os.path.exists(PROMPT_DIR):
            return jsonify({"prompts": [], "message": "プロンプトディレクトリが存在しません。"})
        ...
        raise APIError(f"プロンプト一覧の取得中にエラーが発生しました: {e!s}", 500)
...
def get_prompt(prompt_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise APIError(f"プロンプトの取得中にエラーが発生しました: {e!s}", 500)
...
def delete_prompt(prompt_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise APIError(f"プロンプトの削除中にエラーが発生しました: {e!s}", 500)

    return jsonify({"success": True, "message": f"プロンプト {prompt_id} が削除されました"})
